...
class TestStreamlitAppNavigation:
    ...
    def test_app_loads_successfully(self, page: Page):
        ...
        # Check for title
        expect(
            page.get_by_role("heading", name="SteelWorks Operations")
        ).to_be_visible()
    ...
    def test_navigation_menu_visible(self, page: Page):
        ...
        expect(
            sidebar.get_by_text("Production Line Quality", exact=True)
        ).to_be_visible()
        ...
        expect(
            sidebar.get_by_text("Lot Details (Drill-down)", exact=True)
        ).to_be_visible()
...
class TestProductionLineQualityPage:
    ...
    def test_production_line_page_loads(self, page: Page):
        ...
        expect(
            page.get_by_role("heading", name="Production Line Quality Analysis")
        ).to_be_visible()
...
class TestDefectTrendsPage:
    ...
    def test_defect_trends_page_loads(self, page: Page):
        ...
        expect(
            page.get_by_role("heading", name="Defect Trend Analysis")
        ).to_be_visible()
...
class TestShipmentStatusPage:
    ...
    def test_shipment_status_page_loads(self, page: Page):
        ...
        page.get_by_test_id("stSidebar").get_by_text(
            "Shipment Status", exact=True
        ).click()
        page.wait_for_timeout(1000)

        expect(
            page.get_by_role("heading", name="Shipment Status Review")
        ).to_be_visible()
...
class TestLotDetailsPage:
    ...
    def test_lot_details_page_loads(self, page: Page):
        ...
        expect(
            page.get_by_role("heading", name="Lot Details & Cross-Functional Review")
        ).to_be_visible()
